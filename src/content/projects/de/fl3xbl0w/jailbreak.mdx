...
- Eine USB-Tastatur (kabelgebunden oder drahtlos über einen USB-Dongle) mit "Medientasten" für Home, Back.
...
_Bluetooth-Tastaturen scheinen sich nicht über JRNY koppeln zu lassen._
...
<Image
  src={ConsoleUSB}
  alt="Zeigt den Standort des USB-Ports der Konsole an"
  format="webp"
/>
...
Ok, jetzt der lustige Teil: Dieses Dialogfeld gehört zum "Fehlerüberprüfungssystem" von Standard-Android. Wir haben gerade "Android dazu gebracht zu denken", dass NautilusLauncher defekt ist (gut!). Lassen wir uns durch dieses Dialogfeld ausnutzen.

Sobald es erscheint, tippe auf "App-Info". Durch Klicken auf "App-Info" öffnen wir die Einstellungs-App (die in der Ablehnungsliste von `AppMonitorService` steht). Sie wird sich in etwa einer Sekunde schließen, also müssen wir schnell sein! Tippe auf den Button "STOPP ERZWINGEN" und dann auf "OK".
...
_Bei der ersten Remote-Verbindung über adb wird eine Bestätigung auf dem Bildschirm angefordert._

Lassen Sie uns eine leere Datei in `/sdcard/Pictures/` namens `nautilus` erstellen. Mehr über das "Warum" kannst du in [Bowflex Kuriositäten](/de/projects/fl3xbl0w/quirks) lesen.

```sh
> adb shell touch /sdcard/Pictures/nautilus
```

Jetzt stellen wir einige Funktionalitäten wieder her. Sende die folgenden Befehle über dein Terminal:
...
Es gibt auch eine weitere "Sicherheitssperre", einen Vordergrunddienst in der `NautilusLauncher`-Apk, genannt `AppMonitorService`. Wenn man die Apk dekompiliert, kann man drei interessante Variablen sehen:
...
Und so schützt dieser Dienst "aktiv" das Sperrsystem. Bei jedem Schließen der App (durch Halten der "Back"-Taste) wird versucht, JRNY wieder zu öffnen. Durch das Erzwingen des Schließens deaktivieren wir auch alle Vordergrunddienste.

_(Die Tippfehler, die du in den Code-Backups sehen kannst, stammen von den Nautilus-Entwicklern selbst, nicht von mir)_
