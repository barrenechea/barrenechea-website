...
Android has a shortcut to close the current app through the "Back" button if you hold it for a second or so.
...
Ok, now the fun part: That dialog belongs to Android's stock "crash verification system". We just "forced Android to think" that NautilusLauncher is bad (good!). Let's exploit through that dialog.
