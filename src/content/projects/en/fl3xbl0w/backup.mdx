...
Make sure you're already connected to the Console:
...
First, get the list of all packages installed on the Console (just for you to have a reference):
...
This seems to be able to backup _some (not sure if all)_ AppData.

_Note that not all installed apps generate AppData, or we, as the `shell` user, may not have permissions to backup everything._
...
_These backup and extraction methods are from [this Gist](https://gist.github.com/AnatomicJC/e773dd55ae60ab0b2d6dd2351eb977c1). I've only tested the Backup methods listed there, which are safe to play. **If you play with the Gist Restore, you're on your own**. Let us know if it works tho!_
