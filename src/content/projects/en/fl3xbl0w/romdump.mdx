...
<Image
  src={ConsoleMounted}
  alt="Console mounted with the back cover removed"
  format="webp"
/>
...
You can keep the Android Tool Release application open all the time as a reference to check in which mode the console is.

With the machine turned on in "normal mode" and connected to a computer, it is identified as `NFTM-LAR`:
...
With the machine turned off, connected via USB to a computer, holding down the "Recovery button" and turning it on, it should boot in "Loader mode". The machine will "sound", but the screen will remain static. _You can turn off, wait a minute, turn on and the machine will boot normally._
