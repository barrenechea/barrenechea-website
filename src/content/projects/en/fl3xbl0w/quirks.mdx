...
_`DOMAIN_PIN` seems to be a real pin code as it is a six-digit number._
...
I found references to `com.redbend.client` [here](https://www.mdxers.org/threads/com-redbend-clent-has-stopped.175897/), and I found it quite funny to see that it was crashing in a car. I hope it doesn't fail that often in our machines as well.
...
It seemed pretty standard, and a [post](https://community.home-assistant.io/t/ble-heartrate-monitor/300354/39) from the Home Assistant Community had already figured this out (not the same model, but it seems that the Heart Rate Monitor uses a standard protocol). I have created an ESPHome configuration file [here](https://github.com/barrenechea/esphome-config-files/blob/master/bowflex-hr-monitor.yaml) as a proof of concept:
