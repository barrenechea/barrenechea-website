...
imgAlt: "Image générée par Firefly Image 2 avec le texte : art primé, un arbre ramifié fait de rayons de lumière et d'énergie, dramatique, impactant, coloré, haute définition, 4k uhd"
...
Ces derniers jours, j'ai internationalisé chaque coin du site, en attribuant des valeurs dans un format clé/valeur pour l'interface, et en traduisant le contenu des projets et des publications de manière automatisée en utilisant des LLMs (Modèles de Langage de Grande Taille).
...
Ce site se concentre principalement sur le contenu des publications et, en termes d'interface, est assez simple. La majorité du texte statique était sur la page d'accueil, donc j'ai commencé par extraire chaque valeur dans des objets clé/valeur, permettant à la vue de se rendre de la même manière.
...
En anglais, je pouvais utiliser les mêmes valeurs pour les projets et les publications, mais en espagnol non.

Dans un monde idéal, j'utiliserais un langage neutre en termes de genre. J'aurais pu opter pour "Proyectos recientes" et "Publicaciones recientes", ce qui me permettrait de recycler le mot. Cependant, pour maintenir la cohérence visuelle de la page, j'avais besoin que la phrase se termine par "proyectos" ou "publicaciones". Après quelques ajustements, j'ai réussi à la traduire complètement dans toutes les vues (l'index, les pages de navigation et les modèles utilisant le contenu).

Et il y avait un problème avec le composant de sélection de langue que j'avais utilisé qui me dérangeait beaucoup. Le composant nécessitait la classe `absolute` pour positionner des objets en dessous de lui, ce qui a causé le non-respect de la marge de la page en cas de texte très long :

<div align="center">
  <Image
    src={OldNavbar}
    alt="Capture d'écran de l'ancienne barre de navigation, avec le sélecteur de langue dans le coin supérieur droit dépassant la limite de la marge"
    format="webp"
  />
...
En ajustant le contenu du site (projets et publications) à son chemin correspondant en anglais, le site ne trouvait pas le contenu en espagnol, ce qui résultait en une page sans contenu navigable. Les premiers tests, je les ai faits en traduisant manuellement avec l'API d'OpenAI, puis j'ai automatisé le processus pour faciliter les choses.

_Vous pourrez voir plus de détails dans une prochaine publication sur l'utilisation des LLMs en production._

Après avoir fini de traduire les projets et les publications en espagnol, j'ai fait quelques ajustements aux traductions résultantes. J'ai établi l'espagnol comme standard de référence pour le processus de traduction automatisé et, après les ajustements nécessaires, j'ai régénéré le contenu en anglais.
